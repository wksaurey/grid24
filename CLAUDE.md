...
**Clipboard v0 SHIPPED 2026-07-19 (lab; four same-day iterations → the TWO-TIER SLOTS model, Kolter's design):** 4×2 grid, **flick ⇢ toggles the layer** (in from anywhere, out from within; swipe-down also exits). **Row 0 = two live SOURCE slots, read-only:** `⌁` newest system clipboard (primary-clip listener) and `⌫` the delete buffer (last selection destroyed by typing/DELETE — the stash, which never writes the system clipboard; deviation 5 amended). Sources always paste on tap, including over a live selection (replaced text lands in the delete buffer — reversible). **Rows 1-3 = six manual REGISTERS that only explicit stores touch** (no eviction policy — ownership makes one unnecessary): with a live selection, tap a register = store (copy) the selection into it, hold = cut into it (empties show "＋ store"); without one, tap = paste. All actions auto-return to alpha. Registers are keyboard-local; **flick ⇠ = system-clipboard copy** (performContextMenuAction, travels to other apps, shows up in `⌁`). Superseded same-day experiments (git history): swipe-down-thirds entry, action-button row, full-width selection slot, single-pool history with eviction. Still v2: persistence, per-register clear, pinning-as-macros synergy; terminals have no clip-layer entry (raw flicks stay arrows).
...
| Quick flick ⇠/⇢ | travel > TAP_T, duration < FLICK_MS, horizontal-dominant | **copy selection / open the clipboard layer** (2026-07-19 redesign; the prototype's ±1 cursor nudge and selection-collapse are removed — slow drags own cursor movement; raw-key hosts keep arrow flicks; a flick that briefly engaged the drag engine restores the touch-down selection first) |
...
| Swipe ↓ on alpha | same, start-x in left half / right half | open symbol layer / open number layer (clipboard layer opens via **flick ⇢**, not swipe-down — thirds tried and reverted 2026-07-19) |
